...
_sentinel_sweep: list = []   # deferred protective exits (PEAK_DECAY_10 / RUNNER_DECAY_10) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ flushed once per scan cycle
...
def _load_state():
    ...
    try:
        ...
        log_rows = sb.table("mexc_trade_log").select("*").order("created_at").limit(1000).execute()
        if log_rows.data:
...
def _append_trade_log(trade: dict, exit_price: float, reason: str, pnl: float, r: float):
    ...
    if _mfe_r is None:
        print(f"[ANALYTICS WARN] mfe_r is null for {trade.get('symbol')} - extreme_price={_ep} entry={_entry} sl_dist={_sl_d}")
    if _mae_r is None:
        print(f"[ANALYTICS WARN] mae_r is null for {trade.get('symbol')} - adverse_price={_ap} entry={_entry} sl_dist={_sl_d}")
...
async def _open_trade_log_row(trade: dict):
    """Insert an entry-analytics snapshot into mexc_trade_log at trade-open time.

    If any column is missing, run once in Supabase SQL editor:
      ALTER TABLE mexc_trade_log ADD COLUMN IF NOT EXISTS j15m_entry       float;
      ALTER TABLE mexc_trade_log ADD COLUMN IF NOT EXISTS j1h_entry        float;
      ALTER TABLE mexc_trade_log ADD COLUMN IF NOT EXISTS stoch_k_entry    float;
      ALTER TABLE mexc_trade_log ADD COLUMN IF NOT EXISTS stoch_d_entry    float;
      ALTER TABLE mexc_trade_log ADD COLUMN IF NOT EXISTS rsi_entry        float;
      ALTER TABLE mexc_trade_log ADD COLUMN IF NOT EXISTS depth_pct_entry  float;
      ALTER TABLE mexc_trade_log ADD COLUMN IF NOT EXISTS chg24h_entry     float;
...
def send_telegram(alert: dict) -> None:
    ...
    msg = (
        f"\U0001F7E0  MEXC \u00B7 {direction} {sym} \u00B7 {leverage}x\n"
        "\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\n"
...
async def _write_peak_shadow_row(key: str, trade: dict, reason: str, final_pnl: float) -> None:
    try:
        ...
        _session_ps = trade.get("session") or (_get_session(opened_at) if opened_at else None)
        _psh_row = {
            "venue":                  "mexc",
...
def _do_close_trade(key: str, trade: dict, exit_price: float, reason: str):
    ...
    if reason in ("KILL", "DEAD_TRADE_KILL"):
        ...
        if _scanner_mod._consec_adverse[_dir_key_c] == 3:
            _halt_msg = (
                f"⛔ MEXC {direction} DIRECTION HALTED\n"
                f"3 consecutive kills — {direction} entries suspended\n"
                f"Resumes when BTC regime = CLEAR\n"
                f"Session: {get_session_name()}")
            print(f"[CONSEC_HALT] MEXC {direction} halted — 3 consecutive kills")
            if TELEGRAM_ENABLED:
                threading.Thread(
...
def _flush_sentinel_sweep() -> None:
    ...
    if len(exits) == 1:
        ...
        _tg_post(header + " \u00B7 " + str(len(exits)) + " exits \u00B7 MEXC"
                 + "\n" + " \u00B7 ".join(parts)
                 + "\n" + "\u2500" * 32
        ...
        for key, trade in list(app_state.open_trades.items()):
            try:
                ...
                if _px_age > 90:
                    ...
                    _fresh_px = None
                    try:
                        _fresh_px = await mexc_client.get_price(sym)
                ...
                if not PAPER_MODE and _extclose_tick % 5 == 0:
                    try:
                        _exsz = await asyncio.to_thread(_mexc_api.get_open_position_size, sym)
                ...
                if _ps_wt and _cpnl > 0 and _ar_pre.get("be_armed") and tp1_hit:
                    if is_short:
                        ...
                        if _bw and _bw["dist_pct"] <= 0.30:
                            print(f"[WALL_TP] MEXC {sym} SHORT"
                                  f" wall={_bw['price']:.5f}"
                ...
                if (not _pp_suppressed
                    ...
                    _pp_r = _cpnl / _dr_ac
                    print(f"[PEAK_PROTECT] MEXC {sym} {direction}"
                ...
                if (not _sh.get("be_armed", False)
                    ...
                    for _bte_age, _bte_thr in (
                        ...
                        if _elapsed >= _bte_age and _bte_mfe_r < _bte_thr:
                            ...
                            break
                    if _bte_fired:
                        print(f"[BAD_TRADE_EXIT] MEXC {sym} {direction}"
                              f" elapsed={_elapsed:.0f}s"
...
async def lifespan(app: FastAPI):
    ...
    _sentinel_mod.init("MEXC", _get_supabase)
    ...
    print("  ALTER TABLE mexc_trade_log ADD COLUMN IF NOT EXISTS stoch_k_entry    float;")
    print("  ALTER TABLE mexc_trade_log ADD COLUMN IF NOT EXISTS stoch_d_entry    float;")
    print("  ALTER TABLE mexc_trade_log ADD COLUMN IF NOT EXISTS rsi_entry        float;")
    print("  ALTER TABLE mexc_trade_log ADD COLUMN IF NOT EXISTS depth_pct_entry  float;")
    print("  ALTER TABLE mexc_trade_log ADD COLUMN IF NOT EXISTS chg24h_entry     float;")
    ...
    balance_task = asyncio.create_task(_supervised(_mexc_balance_loop,    "mexc_balance_loop"))
    if _digest_task is not None and not _digest_task.done():
        _digest_task.cancel()
    yield
    scan_task.cancel()
...
async def get_pair(symbol: str):
    ...
    stoch_k_fast      = ps.get("stoch_k_fast",      50.0)
...
async def live_brief(symbol: str, direction: str):
    ...
    if   btc_j1h > 80.0:              btc_regime = "LONG_BLOCKED"
    elif btc_j1h < 20.0:              btc_regime = "SHORT_BLOCKED"
    elif 40.0 <= btc_j1h <= 60.0:    btc_regime = "NEUTRAL_BLOCK"
    else:                              btc_regime = "CLEAR"
    ...
    if from_ts is not None and to_ts is not None:
        ...
        if sb is not None:
            try:
                ...
                sb.table("mexc_trade_log").delete() \
